...
export class SessionReplayIntegration implements Integration {
    ...
    /**
     * 启动路由监听
     * 劫持 history API 和监听 hash 变化，路由切换时立即触发 FullSnapshot
     * 支持 history 模式 (pushState/replaceState/popstate) 和 hash 模式 (hashchange)
     */
    private startUrlMonitoring(): void {
        if (typeof window === 'undefined') return

        // 路由变化时的统一处理函数
        // 通过停止并重启录制来触发新的 Meta + FullSnapshot
        const handleRouteChange = () => {
            ...
            // 重新开始录制，会自动生成新的 Meta + FullSnapshot
            this.stopRecording = record({
                emit: (event, isCheckout) => {
                    this.handleEvent(event, isCheckout)
                },
        ...
        // ========== History 模式：监听 popstate（浏览器前进/后退）==========
